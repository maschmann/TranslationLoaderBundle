 ...
 *
 * This file was copied from Symfony 2.3 to keep backward compatibility with
 * Symfony 2.2.
 *
 * @author Johannes Schmitt
 */
class LegacyRegisterListenersPass implements CompilerPassInterface
{
    /**
     * @var string
     */
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds($this->subscriberTag) as $id => $attributes) {
            ...
            if (!$def->isPublic()) {
                throw new \InvalidArgumentException(
                    sprintf('The service "%s" must be public as event subscribers are lazy-loaded.', $id)
                );
            }
            ...
            if (!$refClass->implementsInterface($interface)) {
                throw new \InvalidArgumentException(
                    sprintf('Service "%s" must implement interface "%s".', $id, $interface)
                );
            }
